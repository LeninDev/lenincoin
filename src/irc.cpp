...
// Copyright (c) 2011-2012 Lenin Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #LeninTEST0\r");
            Send(hSocket, "WHO #LeninTEST0\r");
        } else {
            // randomly join #Lenin00-#Lenin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #Lenin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Lenin%02d\r", channel_number).c_str());
        }
