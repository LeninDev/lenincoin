...
const string strMessageMagic = "Lenin Signed Message:\n";
...
{
    // Lenin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // Lenin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LeninMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LeninMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LeninMiner(CWallet *pwallet)
{
    printf("LeninMiner not supported!\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Lenin-miner");
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LeninMiner, pwallet));
}
